...
import com.cy.pro.utils.file.PoiUtil;
...
public class ContractPrint {
    ...
    public void print(Contract contract, String path, HttpServletResponse response) throws Exception {
        //相同厂家的信息一起打印
        Set<ContractProduct> oList = contract.getCps();
        ...
        String oldFactory = "";
        List<ContractProduct> cps = new ArrayList<>(0);
        for (ContractProduct item
                : oList) {
            cps.add(item);
        }
        for (int i = 0; i < cps.size(); i++) {
            oProduct = cps.get(i);    //获得货物
            pageMap = new HashMap();    //每页的内容

            pageMap.put("Offeror", "收 购 方：" + contract.getOfferor());
            pageMap.put("Factory", "生产工厂：" + oProduct.getFactory().getFactory_name());
            ...
            oldFactory = oProduct.getFactory().getFactory_name();
            ...
            if (contract.getPrint_style().equals("2")) {
                ...
                if (i < oList.size()) {
                    ...
                    if (oProduct.getFactory().getFactory_name().equals(oldFactory)) {    //厂家不同另起新页打印，除去第一次的比较
